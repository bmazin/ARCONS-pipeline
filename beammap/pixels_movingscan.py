...
path = '/home/pszypryt/Scratchwork/beammap/'
...
goodpixtag=np.empty(0,dtype='|S10')
...
#        ypos -= 1
    ...
    goodpixtag=np.append(goodpixtag,pixtag[goodpix])
...
#    print freqvals[idx[i]], xpix[idx[i]], ypix[idx[i]]
    ...
    f.write(str(freqvals[idx[i]]) + '\t' + str(attenvals[idx[i]]) +'\t' + str(xpix[idx[i]]) + '\t' + str(ypix[idx[i]]) +'\t' + goodpixtag[idx[i]] + '\n')
    f.close()
