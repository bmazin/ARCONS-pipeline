...
class StartQt4(QMainWindow):
	...
	def make_image(self):
		...
		all_photons = []
		for j in range(self.total_pix):
			all_photons.append([])
		
		if self.dark_subtraction == True:
			darkrate = zeros((32,32))
		...
		indices = sort(reshape(photon_count,1024))
		#print indices
		brightest = int(self.ui.satpercent.value()*1024)
		#print brightest
	...
	def display_image(self):
		...
		if isfile(self.imagefile):
			pix = self.makepixmap(self.imagefile, scalex=10, scaley=10)
			#display pixmap
			...
			if self.histogram_pixel != []:
				for p in self.histogram_pixel:
					...
					self.scene.addRect(10*(x),10*(y),9,9, Qt.blue) # both x10 since image plot is 320x320 pixels
	...
	def start_pixel_select(self,event):
		...
		self.startpy = (self.nxpix-1)-self.startrawy/10
	...
	def end_pixel_select(self,event):
		...
		self.endpy = (self.nxpix-1)-self.endrawy/10
	...
	def pixel_list(self):
		...
		for element in pix:
				...
				if element not in self.histogram_pixel:
					if element >= 0 and element <= 1023:# and element not in self.bad_pix:
