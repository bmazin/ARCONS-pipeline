...
class PhotonList(IsDescription):
    ...
    waveUpperLimit = Float32Col(dflt=np.nan)           # Error in dec
    ...
    waveOutOfRange = Float32Col(dflt=np.nan)        # Estimated 1-sigma Wavelength error in Angstroms 
...
class CrabList(IsDescription):
    ...
    timeMaskFlag = BoolCol(dflt=False)         # Set if photon is time-masked. True=bad, False=good.
    timeMaskReason = EnumCol(TimeMask.timeMaskReason, 'none', base='uint8')     # Time-mask reason ('none' if not time-masked)
    totalPhase = Float64Col(dflt=0)
    jd = Float64Col(dflt=0)
    bjd = Float64Col(dflt=0)
    grpMax = Float32Col(dflt=0)
    grpMean = Float32Col(dflt=0)
    pulseNumber = UInt32Col(dflt=0)
    phase = Float64Col(dflt=0)
