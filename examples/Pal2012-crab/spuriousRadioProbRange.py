...
def probsOfGRP(nRadioBins=15,strengthMin=.175,startPeakIndex=None,endPeakIndex=None):
    ...
    labels='iBTOA   pulseNumber BTOA    Noise_Offset    Noise_RMS   Max_P2  Mean_P2 Index_P2    TestMax_P2  TestMean_P2 TestIndex_P2'
    ...
    strengthLabel = 'Max_P{}'.format(peakToCheck)
    ...
    peakDetectedIndices = giantDict['Index_P2'][peakDetectionMask]
    noiseDetectedIndices = giantDict['TestIndex_P2'][noiseDetectionMask]
    ...
    print 'indices min max',np.min(noiseDetectedIndices),np.max(noiseDetectedIndices)
    ...
    radioIndexBins = np.arange(1368.5,1395,1)
