...
def probsOfGRP(nRadioBins=15,lowerStrengthHistEdge=.1):
    ...
    peakDetectionMask2 = giantDict[strengthLabel]!= 0.
    ...
    startPeakIndex = 143#np.min(peakDetectedIndices)#1308
    endPeakIndex = 178#np.max(peakDetectedIndices)#1333
    ...
    peakDetectedIndices2 = peakDetectedIndices2[clipPeakIndexMask2]

    peakDetectionMask3 = giantDict[strengthLabel]>= .155
    noiseDetectionMask3 = giantDict['TestMax']>= .155
...
#        print iBin,np.log10(bin),np.log10(noiseDist[iBin]),np.log10(peakDist[iBin]),np.log10(peakDist[iBin]-noiseDist[iBin])
    ...
    grpProb[np.isnan(grpProb)]=1.
    grpProb[grpProb==-np.inf]=0.
    print 'grpProb',grpProb
    ...
    nPoints = 100
    filledStrengthBins = np.linspace(lowerStrengthHistEdge,distBinCenters[-1],nPoints)
    ...
    print 'grpProbAtIndices',grpProbAtIndices
    ax.plot(radioStrengthBinCenters,grpProbAtIndices,'r.')
    ax.set_xlim([.1,.3])
