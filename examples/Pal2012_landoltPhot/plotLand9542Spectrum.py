...
ax.set_ylim(min((curve/tp)[wvls>3800])/2.0,max((curve/tp)[wvls>3800])*1.2)
ax.set_ylim(min((curve/tp)[wvls>3800])/3,max((curve/tp)[wvls>3800])/1.2)
ax.set_xlim(3800,12500)

#put crab photometry points on plot
plt.errorbar(centers,counts,yerr=counterrors,fmt='o')

ax.set_title("Landolt 95-42")
