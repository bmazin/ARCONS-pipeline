...
for k in xrange(nFiles):
    FileName = '/home/srmeeker/scratch/standards/pg0220a_fit_%s.npz'%(fileNum)
    print FileName
    t = np.load(FileName)
...
#data for PG0220+132a
mags = [16.215,16.554,15.771,15.257,14.776,14.046] #simbad
#mags = [15.472, 15.153, 14.216, 13.654, 13.158, 12.386]
centers = [3600,4400,5500,6400,7900,12500]
errors = [0.07,0.07,0.015,0.015,0.048,0.028]
...
ax.set_ylim(min((curve/tp)[wvls>3800])/2.0,max((curve/tp)[wvls>3800])*2.0)
#ax.set_ylim(min((curve/tp)[wvls>3800])/3,max((curve/tp)[wvls>3800])/2.0)
ax.set_xlim(3800,12500)
ax.set_title("PG0220+132a")
