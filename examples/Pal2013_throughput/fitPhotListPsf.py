...
for iFrame in range(0,np.shape(stack)[0]):
    ...
    '''
    apertureMask = aperture(guessX,guessY,radius=100)
    ...
    err = np.ones(np.shape(frame))*10.0
    err[apertureMask==1] = np.inf #weight points closer to the expected psf stronger    
    #err[frame>50] = np.inf #weight pixels above a realistic maximum count rate weaker
    frame[nanMask]=0#set to finite value that will be ignored
    err[nanMask] = np.inf#ignore these data points
    nearDeadCutoff=1#100/15 cps for 4000-6000 angstroms
    err[frame<nearDeadCutoff] = np.inf
    ...
    #minpars = [0,0,29,21,3] #tighter constraint on PSF width to avoid fitting wrong peak if PSF is divided by dead pixels
    maxpars = [4000,5000,250,250,20]
...
print 'saved'
utils.makeMovie(fitImgList,frameTitles=wvls, cbar=True, outName=giffitpsf, normMin=0, normMax=3000)
