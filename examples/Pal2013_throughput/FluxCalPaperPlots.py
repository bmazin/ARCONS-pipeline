...
FileName = '/home/srmeeker/ARCONS-pipeline/examples/Pal2013_throughput/hz21_fit_0.npz'
NumFrames = 31
...
plt.legend(['HZ21 Spectrum','BB Fit','Rebinned Std Spectrum','Resampled Std Spectrum'],'upper right', numpoints=1)
...
multqe = np.array(multqe)*100
plt.errorbar(multqewvls, multqe, color='blue',fmt='o')
print multqe
...
#plt.show()
thruput = curve/y*100
measThru= [thruput[np.array(wvls,int)==4512][0],thruput[np.array(wvls,int)==5548][0],thruput[np.array(wvls,int)==6435][0]]
print measThru
print multqe/measThru
