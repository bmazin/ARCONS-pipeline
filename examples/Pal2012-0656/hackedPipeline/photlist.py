...
def createEmptyPhotonListFile(obsFile,fileName=None):
     ...
     try:
         ...
         plTable = plFile.createTable(plGroup, 'photons', ArconsHeaders.TimedPhotonList, 'Photon List Data', 
                                      filters=bloscFilter) 
                                      #expectedrows=300000)
     except:
         plFile.close()
...
def writePhotonList(obsFile, filename=None, firstSec=0, integrationTime=-1, pixelList=None,
    ...
    try:
        ...
        photListDescription = ArconsHeaders.TimedPhotonList()
        ...
        for iRowRaw in xrange(obsFile.nRow):
            for iColRaw in xrange(obsFile.nCol):
                ...
                if (pixelList == None or (iRowRaw,iColRaw) in pixelList):     #only write photons in good pixels
